...
func formList(args []string) error {
	...
	for _, field := range fields {
		...
		fmt.Printf("%d %s name=%q field_type=%s value=%q kids=%d button_widget_appearance_proof=%t\n", field.Index, object, field.Name, field.FieldType, value, field.KidCount, field.ButtonWidgetAppearanceProof)
	}
	return nil
}
...
func formSet(args []string) error {
	...
	asJSON := fs.Bool("json", false, "write JSON")
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true}, map[string]bool{"format": true, "field": true, "value": true, "match-index": true, "o": true})); err != nil {
		return err
	}
	if fs.NArg() != 1 {
	...
	output, report, verification, err := pdf.ApplyFormFieldEdit(input, *field, *value, selected)
	if err != nil {
		return err
	}
...
func annotList(args []string) error {
	...
	for _, annotation := range annotations {
		...
		fmt.Printf("%d %s subtype=%s contents=%q has_appearance=%t\n", annotation.Index, object, annotation.Subtype, annotation.Contents, annotation.HasAppearance)
	}
	return nil
}
...
func annotSetContents(args []string) error {
	...
	outputPath := fs.String("o", "", "output file")
	asJSON := fs.Bool("json", false, "write JSON")
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true, "remove-appearance": true}, map[string]bool{"format": true, "index": true, "contents": true, "o": true})); err != nil {
		return err
	}
	if fs.NArg() != 1 {
	...
	output, report, verification, err := pdf.ApplyAnnotationContentsEdit(input, *index, *contents, pdf.AnnotationContentsEditOptions{RemoveAppearance: *removeAppearance})
	if err != nil {
		return err
	}
...
func xfaList(args []string) error {
	...
	packets, err := pdf.ListXFAPackets(input)
	if err != nil {
		return err
	}
	...
	for _, packet := range packets {
		...
		fmt.Printf("%d %s label=%s stream=%t length=%d preview=%q\n", packet.Index, object, label, packet.IsStream, packet.TextLength, packet.Preview)
	}
	return nil
}
...
func xfaReplace(args []string) error {
	...
	outputPath := fs.String("o", "", "output file")
	asJSON := fs.Bool("json", false, "write JSON")
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true}, map[string]bool{"format": true, "text": true, "replace": true, "match-index": true, "o": true})); err != nil {
		return err
	}
	if fs.NArg() != 1 {
	...
	output, report, verification, err := pdf.ApplyXFAReplace(input, *text, *replace, selected)
	if err != nil {
		return err
	}
	return writeSemanticEditResult(output, report, verification, *outputPath, *asJSON)
...
func validate(args []string) error {
	...
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true, "fail-on-invalid": true}, map[string]bool{"format": true})); err != nil {
		return err
	}
	if fs.NArg() != 1 {
...
func validationWarnings(tree *core.Tree) []string {
	...
	if boolBoundary(boundaries, "has_acroform") {
		warnings = append(warnings, "PDF boundary detected: AcroForm field values require form set; widget appearances are not regenerated")
	}
	if boolBoundary(boundaries, "has_annotations") {
		warnings = append(warnings, "PDF boundary detected: annotations can update /Contents only; appearance regeneration is not implemented")
	}
	if boolBoundary(boundaries, "has_font_markers") || boolBoundary(boundaries, "has_cmap_markers") || boolBoundary(boundaries, "has_tounicode_cmap") || boolBoundary(boundaries, "has_cid_font_markers") {
		warnings = append(warnings, "PDF boundary detected: font/CMap support is limited to page font-scoped ToUnicode CMaps for simple Tf flows plus one unambiguous fallback; glyph metrics and layout are not verified")
	}
	return warnings
}
...
func query(args []string) error {
	...
	asJSON := fs.Bool("json", false, "write JSON")
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true}, map[string]bool{"format": true, "kind": true, "text": true, "match-index": true, "meta": true})); err != nil {
		return err
	}
	if fs.NArg() != 1 {
...
func edit(args []string) error {
	...
	outputPath := fs.String("o", "", "output file")
	asJSON := fs.Bool("json", false, "write JSON")
	if err := fs.Parse(reorderFlags(args, map[string]bool{"json": true, "allow-signature-invalidation": true}, map[string]bool{"format": true, "kind": true, "text": true, "replace": true, "rewrite": true, "match-index": true, "index": true, "verify": true, "o": true})); err != nil {
		return err
	}
...
func canonicalEdit(input []byte, req canonicalEditRequest) error {
	...
	if req.AllowSignatureInvalidation {
