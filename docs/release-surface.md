...
`signature inspect` is read-only and can validate the PDF byte-range digest against supported CMS/PKCS#7 signed `messageDigest` authenticated attributes. Unsupported or malformed CMS remains explicit with `byte_range_digest_validation_status: "unsupported"`; mismatches report `byte_range_digest_validation_status: "mismatch"`. Certificate trust status is separate and does not imply system-root trust, revocation, timestamp authority validation, signer public-key verification, or re-signing.
...
- AESV3+/public-key/unsupported-crypt-filter encrypted PDFs, default signed-PDF edits, public signature re-signing, system trust-chain/revocation/timestamp validation, broad XFA rendering, bundled OCR/raster recognition, and implicit overlay/stamp fallbacks are not supported.
...
- Filters outside ASCIIHex, ASCII85, RunLength, Flate, LZW, and their standard abbreviations, encrypted AESV3+/public-key/unsupported-crypt-filter shapes, default signed-PDF edits, signature trust-chain validation/re-signing, broad XFA rendering, broad appearance/layout support, and broad font/CMap/layout support remain out of scope.
