...
`signature inspect` is read-only and can validate the PDF byte-range digest against supported CMS/PKCS#7 signed `messageDigest` authenticated attributes. Unsupported or malformed CMS remains explicit with `cryptographic_validation_status: "unsupported"`; mismatches report `byte_range_digest_mismatch`; trust-chain validation, revocation, timestamp authority validation, signer public-key verification, and re-signing are not performed.
...
- AESV3+/public-key/unsupported-crypt-filter encrypted PDFs, default signed-PDF edits, signature trust-chain validation/re-signing, broad XFA rendering, bundled OCR/raster recognition, and implicit overlay/stamp fallbacks are not supported.
